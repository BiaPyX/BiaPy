...
class ImageDataGenerator(tf.keras.utils.Sequence):
    """Custom ImageDataGenerator based on `imgaug <https://github.com/aleju/imgaug-doc>`_
       ...
       cout_size : tuple of floats, optional
           Range to select the size of the areas in % of the corresponding image 
           size. Values between ``0`` and ``1``. E. g. ``(0.2, 0.4)``.
       ...
       cmix_size : tuple of floats, optional
           Range to select the size of the area to paste one image into another. 
           E. g. ``(0.2, 0.4)``.   
       ...
       ::
           ...
           data_gen_args = dict(
               X=X_train, Y=Y_train, batch_size=6, shuffle=True,
               rand_rot=True, vflip=True, hflip=True)

           data_gen_val_args = dict(
               X=X_val, Y=Y_val, batch_size=6, shuffle=True,
               da=False, val=True)
           ...
           data_gen_val_args = dict(
               ...
               shape=(256, 256, 1), shuffle=True, da=False, val=True)
