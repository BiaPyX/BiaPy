...
class Trainer(object):
    ...
    def __init__(self, cfg, job_identifier):
        ...
        if cfg.PROBLEM.TYPE == 'INSTANCE_SEG' and cfg.DATA.CHANNELS != 'B':
            ...
            if cfg.TRAIN.ENABLE and not os.path.isfile(cfg.DATA.TRAIN.INSTANCE_CHANNELS_FILE):                                                    
                print("You select to create {} channels from given instance labels and no file is detected in {}. "     
                      "So let's prepare the data. Notice that, if you do not modify 'DATA.TRAIN.INSTANCE_CHANNELS_FILE' "    
                      "path, this process will be done just once!".format(cfg.DATA.CHANNELS,                            
                      cfg.DATA.TRAIN.INSTANCE_CHANNELS_FILE))                                                                
                self.train_filenames = create_train_val_instance_channels(cfg)
                                                                                                                        
            # Create selected channels for test data once
            if cfg.TEST.ENABLE and not os.path.isfile(cfg.DATA.TEST.INSTANCE_CHANNELS_FILE):                                                   
                print("You select to create {} channels from given instance labels and no file is detected in {}. "         
                      "So let's prepare the data. Notice that, if you do not modify 'DATA.TEST.INSTANCE_CHANNELS_FILE' "   
                      "path, this process will be done just once!".format(cfg.DATA.CHANNELS,                                
                      cfg.DATA.TEST.INSTANCE_CHANNELS_FILE))    
                create_test_instance_channels(cfg)

            print("DATA.TRAIN.PATH changed from {} to {}".format(cfg.DATA.TRAIN.PATH, cfg.DATA.TRAIN.INSTANCE_CHANNELS_FILE))
            print("DATA.TRAIN.MASK_PATH changed from {} to {}".format(cfg.DATA.TRAIN.MASK_PATH, cfg.DATA.TRAIN.INSTANCE_CHANNELS_MASK_FILE))
            opts = ['DATA.TRAIN.PATH', cfg.DATA.TRAIN.INSTANCE_CHANNELS_FILE,
                    'DATA.TRAIN.MASK_PATH', cfg.DATA.TRAIN.INSTANCE_CHANNELS_MASK_FILE]
            if not cfg.DATA.VAL.FROM_TRAIN:
                print("DATA.VAL.PATH changed from {} to {}".format(cfg.DATA.VAL.PATH, cfg.DATA.VAL.INSTANCE_CHANNELS_FILE))
                print("DATA.VAL.MASK_PATH changed from {} to {}".format(cfg.DATA.VAL.MASK_PATH, cfg.DATA.VAL.INSTANCE_CHANNELS_MASK_FILE))
                opts.extend(['DATA.VAL.PATH', cfg.DATA.VAL.INSTANCE_CHANNELS_FILE,
                             'DATA.VAL.MASK_PATH', cfg.DATA.VAL.INSTANCE_CHANNELS_MASK_FILE])
            if cfg.TEST.ENABLE and cfg.DATA.TEST.IN_MEMORY:
                original_test_path = cfg.DATA.TEST.PATH
                print("DATA.TEST.PATH changed from {} to {}".format(cfg.DATA.TEST.PATH, cfg.DATA.TEST.INSTANCE_CHANNELS_FILE))
                opts.extend(['DATA.TEST.PATH', cfg.DATA.TEST.INSTANCE_CHANNELS_FILE])
                if cfg.DATA.TEST.LOAD_GT:
                    original_test_mask_path = cfg.DATA.TEST.MASK_PATH
                    print("DATA.TEST.MASK_PATH changed from {} to {}".format(cfg.DATA.TEST.MASK_PATH, cfg.DATA.TEST.INSTANCE_CHANNELS_MASK_FILE))
                    opts.extend(['DATA.TEST.MASK_PATH', cfg.DATA.TEST.INSTANCE_CHANNELS_MASK_FILE])
        ...
        if cfg.TRAIN.ENABLE:
            ...
            if not cfg.DATA.VAL.FROM_TRAIN:
                if cfg.DATA.VAL.IN_MEMORY:
                    ...
                    if os.path.isdir(cfg.DATA.VAL.PATH):
                        X_val, _, _ = f_name(cfg.DATA.VAL.PATH)                                           
                        Y_val, _, _ = f_name(cfg.DATA.VAL.MASK_PATH)                                           
                    else:
                        X_val = np.load(cfg.DATA.VAL.PATH)
                        Y_val = np.load(cfg.DATA.VAL.MASK_PATH)

                    f_name = crop_data_with_overlap if cfg.PROBLEM.NDIM == '2D' else crop_3D_data_with_overlap
                    if cfg.DATA.PATCH_SIZE != X_val[0].shape:
                        X_val, Y_val = f_name(X_val, cfg.DATA.PATCH_SIZE, data_mask=Y_val, overlap=cfg.DATA.VAL.OVERLAP,
                                              padding=self.cfg.DATA.VAL.PADDING, verbose=False)
                else:
        ...
        if cfg.TEST.ENABLE:
            ...
            if cfg.DATA.TEST.IN_MEMORY:
                ...
                if os.path.isdir(cfg.DATA.TEST.PATH):
                    f_name = load_data_from_dir if cfg.PROBLEM.NDIM == '2D' else load_3d_images_from_dir
                    ...
                    X_test = np.load(cfg.DATA.TEST.PATH)
                    if cfg.DATA.TEST.LOAD_GT:                                                                           
                        print("3) Loading test masks . . .")
                        Y_test = np.load(cfg.DATA.TEST.MASK_PATH)
            else:
    ...
    def test(self):
            ...
            for j in tqdm(range(X.shape[0]), leave=False):
                ...
                if self.cfg.TEST.STATS.PER_PATCH:
                    ...
                    if pred.ndim == 4 and self.cfg.PROBLEM.NDIM == '3D':                                                
                        ...
                        save_tif(pred, self.cfg.PATHS.RESULT_DIR.PER_IMAGE, filenames)
                ...
                if self.cfg.TEST.STATS.FULL_IMG and self.cfg.PROBLEM.NDIM == '2D':
                    ...
                    if gt: _Y, o_test_shape = check_downsample_division(_Y, len(self.cfg.MODEL.FEATURE_MAPS)-1)
                    ...
                    if gt: _Y = _Y[:,:o_test_shape[1],:o_test_shape[2]]
                    ...
                    if pred.ndim == 4 and self.cfg.PROBLEM.NDIM == '3D':
                        ...
                        save_tif(pred, self.cfg.PATHS.RESULT_DIR.FULL_IMAGE, filenames)
        ...
        if self.cfg.TEST.STATS.FULL_IMG and self.cfg.PROBLEM.NDIM == '2D':
            ...
            save_tif(all_pred, self.cfg.PATHS.RESULT_DIR.FULL_POST_PROCESSING)
        ...
        if not self.cfg.MODEL.LOAD_CHECKPOINT: 
