...
def crop_3D_data_with_overlap(data, vol_shape, data_mask=None, overlap=(0,0,0), padding=(0,0,0), verbose=True,
    ...
    if data.shape[0] < vol_shape[0]:    
        raise ValueError("0 axis is smaller than given crop shape: {} vs {}".format(data.shape[0], vol_shape[0]))
    if data.shape[1] < vol_shape[1]:    
        raise ValueError("0 axis is smaller than given crop shape: {} vs {}".format(data.shape[1], vol_shape[1]))
    if data.shape[2] < vol_shape[2]:    
        raise ValueError("0 axis is smaller than given crop shape: {} vs {}".format(data.shape[2], vol_shape[2]))
