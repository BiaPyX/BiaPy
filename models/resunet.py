...
from models.blocks import DoubleResConvBlock, ResUpBlock
    ...
    def __init__(self, image_shape=(256, 256, 1), activation="ELU", feature_maps=[32, 64, 128, 256], drop_values=[0.1,0.1,0.1,0.1],
        ...
        for i in range(self.depth):
            self.down_path.append( 
                DoubleResConvBlock(conv, in_channels, feature_maps[i], k_size, activation, batchnorm_layer,
                    drop_values[i], first_block=True if i==0 else False)
            )
            in_channels = feature_maps[i]

        self.bottleneck = DoubleResConvBlock(conv, in_channels, feature_maps[-1], k_size, activation, batchnorm_layer,
            drop_values[-1])
        ...
        for i in range(self.depth-1, -1, -1):
            self.up_path.append( 
                ResUpBlock(self.ndim, convtranspose, in_channels, feature_maps[i], z_down[i], upsample_layer, 
