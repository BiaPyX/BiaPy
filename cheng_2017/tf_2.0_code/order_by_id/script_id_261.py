...
# Dataset variables
train_path = os.path.join('achucarro', 'reshaped_kasthuri', 'train', 'x')
train_mask_path = os.path.join('achucarro', 'reshaped_kasthuri', 'train', 'y')
test_path = os.path.join('achucarro', 'reshaped_kasthuri', 'test', 'x')
test_mask_path = os.path.join('achucarro', 'reshaped_kasthuri', 'test', 'y')
# Note: train and test dimensions must be the same when training the network and
# making the predictions. If you do not use crop_data() with the arg force_shape
# be sure to take care of this.
img_train_width = 4669
img_train_height = 4669
img_train_channels = 1
img_test_width = 4669
img_test_height = 4669
img_test_channels = 1
...
train_crop_discard_path = os.path.join('data_d', 'achu_r_kas_' + str(d_percentage_value), 'train', 'x')
train_crop_discard_mask_path = os.path.join('data_d', 'achu_r_kas_' + str(d_percentage_value), 'train', 'y')
test_crop_discard_path = os.path.join('data_d', 'achu_r_kas_' + str(d_percentage_value), 'test', 'x')
test_crop_discard_mask_path = os.path.join('data_d', 'achu_r_kas_' + str(d_percentage_value), 'test', 'y')

# Data augmentation variables
normalize_data = True
norm_value_forced = 69.72271479046387
custom_da = False
aug_examples = True
keras_zoom = True
...
det_eval_ge_path = os.path.join('cell_challenge_eval', 'general_achu_r_kas')
...
if load_previous_weights == False:
    ...
    h5_file=os.path.join(h5_dir, 'model.fibsem_255_' + test_id + '.h5')
