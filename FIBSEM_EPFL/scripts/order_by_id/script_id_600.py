...
det_eval_ge_path = os.path.join('cell_challenge_eval', 'general_kas_rfib_' + job_file)
...
if random_crops_in_DA == False:
    ...
    if make_threshold_plots == True:
        ...
        for i in range(0, recons_preds_test.shape[0]):                                       
            im = Image.fromarray(recons_preds_test[i,:,:,0]*255)                            
            im = im.convert('L')                                                
            im.save(os.path.join(result_dir,"recons_preds_test_" + str(i) + ".png")) 
            im = Image.fromarray(Y_test[i,:,:,0]*255)                        
            im = im.convert('L')                                                
