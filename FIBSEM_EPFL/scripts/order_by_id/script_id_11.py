...
X_datagen_val.fit(X_val, augment=True, seed=seedValue)
Y_datagen_val.fit(Y_val, augment=True, seed=seedValue)

# Check a few of generated images 
i=0
for batch in X_datagen.flow(X_train, save_to_dir="aug_x", batch_size=batch_size_value, shuffle=True, seed=seedValue, save_prefix='x', save_format='jpeg'):
    i += 1
    if i > 5:
        break
i=0
for batch in Y_datagen.flow(Y_train, save_to_dir="aug_y", batch_size=batch_size_value, shuffle=True, seed=seedValue, save_prefix='y', save_format='jpeg'):
    i += 1
    if i > 5:
        break

X_train_augmented = X_datagen.flow(X_train, batch_size=batch_size_value, shuffle=True, seed=seedValue)
Y_train_augmented = Y_datagen.flow(Y_train, batch_size=batch_size_value, shuffle=True, seed=seedValue)
X_test_augmented = X_datagen_val.flow(X_val, batch_size=batch_size_value, shuffle=True, seed=seedValue)
Y_test_augmented = Y_datagen_val.flow(Y_val, batch_size=batch_size_value, shuffle=True, seed=seedValue)
...
results = model.fit_generator(train_generator, validation_data=test_generator,
                    ...
                    epochs=50, callbacks=[earlystopper, checkpointer, time_callback])
