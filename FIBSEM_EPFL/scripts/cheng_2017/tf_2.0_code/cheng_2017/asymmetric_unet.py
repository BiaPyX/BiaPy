...
def asymmetric_U_Net(image_shape, activation='elu', numInitChannels=16, 
                     ...
                     optimizer="sgd", lr=0.001, t_downsmp_layer=4, batch_size=6):
    """Create the U-Net
    ...
    for i in range(2):
        channels += 8
        x = encode_block(x, channels, batch_size=batch_size, 
                         t_downsmp_layer=t_downsmp_layer)

    # 1st downsample block
    channels += 8
    x = encode_block(x, channels, batch_size=batch_size, downsample=True,  
                     t_downsmp_layer=t_downsmp_layer)
    ...
    x = encode_block(x, channels, batch_size=batch_size, 
                     downsample=True, t_downsmp_layer=t_downsmp_layer)
    ...
    x = decode_block(x, channels, batch_size=batch_size, upsample=True) 

    # First decode block sequence
    for i in range(4):
        x = decode_block(x, channels, batch_size=batch_size)
    ...
    if optimizer == "sgd":
        opt = tensorflow.keras.optimizers.SGD(lr=lr, momentum=0.99, decay=0.0, 
                                   nesterov=False)
    elif optimizer == "adam":
...
def encode_block(inp_layer, channels, batch_size=4, t_downsmp_layer=4, downsample=False):
        if downsample == True:
            shortcut_padded = Lambda(
                sto_downsampling2d, 
                arguments={'t':t_downsmp_layer, 
...
def decode_block(inp_layer, channels, batch_size=4, upsample=False):
   
    if upsample == True:    
        x = Conv2DTranspose(int(channels), (2, 2), activation=None, 
                            strides=(2, 2)) (inp_layer)
    else:
...
def sto_downsampling2d(x, t=4, batch_size=4):                                                 
    print("x: {} ; {} ; {} ".format(x.shape, x.shape[0], tf.shape(x)[0]))
    N = x.shape[0]
    H = x.shape[1]                                                              
    W = x.shape[2]                                                              
    C = x.shape[3]                                                              
    ...
    if N is None:
        ...
        return x 
        a = np.array([ [(c_rows[i], c_cols[j]) for j in range(sv_w*elem)] for i in range(sv_h*elem) ])
        ta = tf.constant(a, dtype=tf.int32)
        print("ta: {}".format(ta.shape) )
        ta = tf.transpose(tf.stack([ta for i in range(C)]), [1, 2, 0, 3])    
        print("ta: {}".format(ta.shape) )
        ta = tf.pad(ta, [[0,0], [0,0], [0,0], [ 1, 1 ]])                 
        print("ta: {}".format(ta.shape) )
        
        tb = tf.identity(ta)
        print("tb: {}".format(tb.shape) )
        i = tf.constant(0)
        while_condition = lambda i, tb: tf.less(i, tf.shape(x)[0])
        @tf.function
        def body(i, tb):
            tb = tf.stack(tb, ta)
            tf.add(i, 1)
        ...
        # do the loop:
        r = tf.while_loop(while_condition, body, [i, tb])
        print("tb: {}".format(tb.shape) )
        print("r: {}".format(r.shape) )
        
        return tf.gather_nd(x, r)
...
def sto_downsampling2d_mod(x, t=4, batch_size=4):                                   
    N = x.shape[0]                                                              
    H = x.shape[1]                                                              
    W = x.shape[2]                                                              
    C = x.shape[3]                                                              
