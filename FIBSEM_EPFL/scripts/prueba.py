...
X_train, Y_train, X_test, Y_test = load_data(TRAIN_PATH, 
                                             TRAIN_MASK_PATH,
                                             TEST_PATH,
                                             TEST_MASK_PATH,
...
checkpointer = ModelCheckpoint('model.fibsem.h5', verbose=1,
                               save_best_only=True)
results = model.fit(X_train, Y_train, validation_split=0.1, batch_size=1,
...
r= compute_iou(preds_test, Y_test)
print("IOU calculado con la funcion: " + str(r))
comp1 = np.array(preds_test_t, dtype=bool)
comp2 = np.array(Y_test, dtype=bool)
overlap = comp1 * comp2
union = comp1 + comp2
IOU = overlap.sum()/float(union.sum())
print("IOU calculado a mano: " + str(IOU)) 
print("IOU calculado con tf: " + str(score[1]))
