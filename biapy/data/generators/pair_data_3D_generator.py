...
class Pair3DImageDataGenerator(PairBaseDataGenerator):
    ...
    def save_aug_samples(self, img, mask, orig_images, i, pos, out_dir, point_dict):
        ...
        if self.random_crops_in_DA and self.prob_map is not None and i == 0:
            ...
            aux = read_img_as_ndarray(self.X.dataset_info[self.X.sample_list[s_idx].fid].path, is_3d=True)
            auxm = read_img_as_ndarray(self.Y.dataset_info[self.Y.sample_list[s_idx].fid].path, is_3d=True)
            if aux.max() < 1:
                aux = aux * 255
            if auxm.max() == 1:
                auxm = auxm * 255
            aux = aux.astype(np.uint8)
            auxm = auxm.astype(np.uint8)
