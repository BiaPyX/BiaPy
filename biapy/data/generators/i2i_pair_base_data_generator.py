...
from biapy.utils.util import img_to_onehot_encoding, pad_and_reflect
from biapy.data.generators.augmentors import *
from biapy.data.pre_processing import normalize, norm_range01

class I2IPairBaseDataGenerator(Dataset, metaclass=ABCMeta):
    """
    ...
    def __init__(self, ndim, X, Y, seed=0, in_memory=True, data_paths=None, da=True, da_prob=0.5, rotation90=False, 
        ...
        if not in_memory:
            ...
            if self.Y_provided:
                ...
                sample_weights = None
                if os.path.exists(os.path.join(data_paths[1],"sample_weigths.csv")):
                    sample_weights = pd.read_csv(os.path.join(data_paths[1],"sample_weigths.csv"))
                c = 0
                    ...
                    if self.val:    
                        for j in range(len(samples)):
                            ...
                            if sample_weights is not None:
                                self.data[f"sample_{c}"]["weight"] = float(sample_weights[sample_weights["filename"]==self.data_mask_path[i]]["weight"].iloc[0])
                            else:
                                self.data[f"sample_{c}"]["weight"] = 1.
                            c += 1
                    else:
                        self.data[f"sample_{c}"] = {}
    ...
    def load_sample(self, _idx):
        ...
        if self.in_memory:
            ...
            weight = self.data[f"sample_{idx}"]["weight"]
    ...
    def __getitem__(self, index):
        ...
        """
        img, mask, weight =  self.load_sample(index)
        ...
        if self.da:
            ...
            if self.cutmix:
                ...
                e_img, e_mask, _ =  self.load_sample(extra_img)
    ...
    def get_transformed_samples(self, num_examples, random_images=True, save_to_dir=True, out_dir='aug', train=False,
        ...
        for i in tqdm(range(num_examples)):
            ...
            img, mask, _ = self.load_sample(pos)
            if save_to_dir:
                orig_images = {}
