...
def export_model_to_bmz(cfg, jobname, model, test_input, test_output):
    """
    ...
    output_sample_path = os.path.join(cfg.PATHS.RESULT_DIR.BMZ_BUILD, "test-output.npy")
    ...
    if cfg.MODEL.BMZ.EXPORT_MODEL.NAME != "":
        model_name = cfg.MODEL.BMZ.EXPORT_MODEL.NAME
    else:
        if cfg.MODEL.SOURCE == "biapy":
            model_name = "my_"+cfg.MODEL.ARCHITECTURE+"_"+cfg.PROBLEM.NDIM
        elif cfg.MODEL.SOURCE == "torchvision":
            model_name = "my_"+cfg.MODEL.TORCHVISION_MODEL_NAME+"_"+cfg.PROBLEM.NDIM
        else:
            model_name = "my_BMZ_"+cfg.MODEL.BMZ.SOURCE_MODEL_DOI
    ...
    postprocessing = None
    # if cfg.PROBLEM.TYPE in ['SEMANTIC_SEG', 'DETECTION', "SUPER_RESOLUTION", "SELF_SUPERVISED"]:
    #     postprocessing = [{"name": "binarize", "kwargs": {"threshold": 0.5}}]

    tags = cfg.MODEL.BMZ.EXPORT_MODEL.TAGS[0]
    if 'dims' not in tags:
        tags['dims'] = cfg.PROBLEM.NDIM.lower()
    ...
    # Change dir as the building process copies to the current directory copies of the files used to create the BMZ model
    cwd = os.getcwd()
    os.chdir(cfg.PATHS.RESULT_DIR.BMZ_BUILD)

    build_model(
        # the weight file and the type of the weights
        weight_uri=get_checkpoint_path(cfg, jobname),
        ...
        output_path=os.path.join(cfg.PATHS.RESULT_DIR.BMZ_BUILD, "model.zip"),
        name=model_name,
        description=cfg.MODEL.BMZ.EXPORT_MODEL.DESCRIPTION,
        # additional metadata about authors, licenses, citation etc.
        authors=cfg.MODEL.BMZ.EXPORT_MODEL.AUTHORS,
        license=cfg.MODEL.BMZ.EXPORT_MODEL.LICENSE,
        documentation=cfg.MODEL.BMZ.EXPORT_MODEL.DOCUMENTATION,
        tags=cfg.MODEL.BMZ.EXPORT_MODEL.TAGS,  
        cite=cfg.MODEL.BMZ.EXPORT_MODEL.CITE,
        preprocessing=preprocessing,
        postprocessing=postprocessing,
    )

    my_model = load_resource_description(os.path.join(cfg.PATHS.RESULT_DIR.BMZ_BUILD, "model.zip") )
    test_model(my_model)
