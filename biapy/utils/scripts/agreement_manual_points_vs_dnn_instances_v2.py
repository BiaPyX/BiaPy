...
# python -u /data/dfranco/BiaPy/biapy/utils/scripts/agreement_manual_points_vs_dnn_instances_v2.py  --input_file_dir /data/dfranco/datasets/lesion_medular/neuron_seg/neuron_final_test --input_instance_dir /data/dfranco/exp_results/lesion_medular/lesion_medular_neuron22/results/lesion_medular_neuron22_1/per_image_instances/ --input_instance_dir2 /data/dfranco/datasets/lesion_medular/neuron_seg/neuron_final_test/cellSense/OUT_EDT
...
def make_single_figure(
    ...
    """
    Compute class distributions for Manual, SpineDL-Neuron #1, and CellSense:
    ...
    nn2_base_to_file = {os.path.splitext(fname)[0]: fname for fname in nn_imgs_2.keys()}
    nn1_basenames = set(nn1_base_to_file.keys())
    nn2_basenames = set(nn2_base_to_file.keys())

    # --- basenames present in GT (from expert XMLs) ---
    gt_basenames = set(experts_df["file"].unique())

    # --- intersection only (GT ∩ DNN1 ∩ CellSense) ---
    common = sorted(gt_basenames.intersection(nn1_basenames).intersection(nn2_basenames))
    if not common:
    ...
    for base in tqdm(common, desc="Processing overlapping sections"):
        ...
        # ----- CellSense vs Manual -----
        nn_mask_2 = nn_imgs_2[nn2_base_to_file[base]]
        dist2inst_2 = edt.edt((nn_mask_2 == 0).astype(np.uint8))
        dnn2_counts, _ = compute_dnn_freqs_for_file(
            df_file, nn_mask_2, tol_px,
            man_clusters=man_clusters,
            dist_to_instances=dist2inst_2,
            dist_to_clicks=dist2clicks,
        )
    ...
    dnn2_mean = np.mean(np.stack(per_file_dnn2, axis=0), axis=0)
    ...
    fig.add_bar(
        ...
        x=x,
        y=dnn2_mean.tolist(),
        text=[f"{v:.1f}" for v in dnn2_mean], 
        textfont=dict(size=10),
        textposition="outside",
        hovertemplate="Class %{x}<br>CellSense: %{y:.2f}<extra></extra>",
    )
    max_y = max(manual_mean.max(), dnn1_mean.max(), dnn2_mean.max())
    fig.update_layout(
        barmode="group",
        title=(
        ...
        bargap=0.2,
        margin=dict(t=150),
        yaxis=dict(range=[0, max_y * 1.2]),  # add 20% headroom if needed
    )
...
def main():
    ...
    print("Loading CellSense #2 instances …")
    nn_imgs_2 = load_instance_masks(nn_dir_2)
