...
class U_NeXt_V1(nn.Module):
    ...
    def __init__(self, image_shape=(256, 256, 1), feature_maps=[32,64,128,256], upsample_layer="convtranspose", z_down=[2,2,2,2], 
                 n_classes=1, output_channels="BC", upsampling_factor=(), upsampling_position="pre", stochastic_depth_prob=0.1, layer_scale=1e-6, 
                 cn_layers=[2,2,2,2]):
        ...
        for i in range(self.depth):
            ...
            for _ in range(cn_layers[i]): 
                ...
                stage.append(ConvNeXtBlock_V1(self.ndim, conv, feature_maps[i], layer_scale, sd_prob, layer_norm))
                stage_block_id += 1
            ...
            self.downsample_layers.append(
                nn.Sequential(
                    ...
                    post_ln_permutation,
                    conv(feature_maps[i], feature_maps[i+1], kernel_size=mpool, stride=mpool),
                )
            )
        ...
        for i in range(self.depth-1, -1, -1):
            self.up_path.append(
                UpConvNeXtBlock_V1(self.ndim, convtranspose, in_channels, feature_maps[i], z_down[i], upsample_layer, 
                    conv, attention_gate=False, cn_layers=cn_layers[i],
                    sd_probs=sd_probs[i], layer_scale=layer_scale, layer_norm=layer_norm))
