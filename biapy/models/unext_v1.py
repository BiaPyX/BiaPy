...
from biapy.models.blocks import UpConvNeXtBlock_V1, ConvNeXtBlock_V1, prepare_activation_layers
...
class U_NeXt_V1(nn.Module):
    ...
    def __init__(
        ...
        layer_scale : float, optional
            Layer Scale parameter for ConvNeXt blocks. Defaults to 1e-6.
        cn_layers : List[int], optional
            Number of ConvNeXt blocks per level. Defaults to `[2, 2, 2, 2]`.
    ...
    def forward(self, x) -> Dict | torch.Tensor:
        """
        Perform the forward pass of the U-NeXt_V1 model.

        The input `x` first undergoes optional pre-upsampling for super-resolution.
        It then passes through the ConvNeXt-style encoder path (stem, ConvNeXt blocks,
        ...
        and processes them through `UpConvNeXtBlock_V1` modules. Finally, optional
