...
class ResUNetPlusPlus(nn.Module):
    ...
    def __init__(
        ...
        mpool = (z_down[0], self.yx_down[0], self.yx_down[0]) if self.ndim == 3 else (self.yx_down[0], self.yx_down[0])
    ...
    def forward(self, x) -> Dict | torch.Tensor:
        ...
        x_bot = self.aspp_bridge(x)

        # Decoder
        feats = []
        for j in range(self.num_decoders):
            x = x_bot
            for i, layers in enumerate(zip(self.attentions[j], self.up_paths[j])):
                att, up = layers
                x = att(blocks[-i - 2], x)
                x = up(x, blocks[-i - 2])
            x = self.aspp_out[j](x)
