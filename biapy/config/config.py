...
class Config:
    ...
    def __init__(self, job_dir: str, job_identifier: str):
        ...
        # Cellpose only. Expected cell diameter (px) in the TEST data (test time only; training always
        # rescales per image). The input is rescaled by DIAM_MEAN / DIAMETER, predicted, and resized
        # back, with niter = (DIAMETER / DIAM_MEAN) * 200. DIAMETER == 0 estimates it per image
        # (TEST_DOUBLE_INFERENCE), else uses the training-set median. Ignored when TYPE is 'omnipose'.
        # Default: 0.0.
        _C.PROBLEM.INSTANCE_SEG.CELLPOSE.DIAMETER = 0.0
        # Cellpose only. Reference cell diameter (px) the flow model is trained at (30 'cyto', 17
        ...
        # 'torchvision' models. Default: True.
        _C.PROBLEM.INSTANCE_SEG.CELLPOSE.TEST_DOUBLE_INFERENCE = True
        # Omnipose only. DBSCAN eps radius (px): convergence positions within this distance join the
        # same cluster. Large enough to stitch a cell's skeleton, small enough not to bridge touching
        # cells. Ignored when TYPE is 'cellpose'. Default: 5.0.
