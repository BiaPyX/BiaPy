...
def flow_target_scale(channel_extra_opts: Optional[Dict]) -> float:
    """
    Scale applied to Cellpose/Omnipose flow *targets* (Gv/Gh/Gz) at loss/metric time.

    Neither Cellpose nor Omnipose bakes this factor into the stored flow field; both multiply the flow
    target by 5 inside the loss (Cellpose: ``veci = 5.*flow``; Omnipose: ``lbl[:,-dim:] = mu*5.0`` in
    ``batch_labels``). The gradient type is read from the same place the flow-field builder uses
    (``DATA_CHANNELS_EXTRA_OPTS['Gv']['gradient_type']``), so the loss/metric always agree with how the
    stored GT was produced.
    """
...
class multiple_metrics:
    ...
    def __init__(
        ...
        # range here so the L1 metric compares like-for-like (see flow_target_scale).
        self.flow_target_scale = flow_target_scale(channel_extra_opts)
    ...
    def __call__(self, y_pred, y_true):
        ...
        for idx, pd in enumerate(_y_pred):
            ...
            for i, channel in enumerate(self.out_channels):
                ...
                mask = None
                if channel in ("Gv", "Gh", "Gz"):
                    # Scale the flow target by flow_target_scale here and compute the MSE over the
                    # whole patch (background targets are 0, driving predictions to 0 outside cells),
                    # so leave mask=None.
                    y_true_slice = y_true_slice * self.flow_target_scale
                    mask = None
...
def rfft_highfreq_loss(pred, target, eps=1e-9):
    """Frequency-domain loss on the real-FFT half-spectrum, masked to emphasize high frequencies.

    Similar intent to :func:`fft_highfreq_loss`, computed on the cheaper real-input half-spectrum
    (``torch.fft.rfft2``/``rfftn``) instead of the full, fftshifted spectrum, with two fixes for
    numerical scale next to the pixel-domain loss terms: an "ortho"-normalized FFT (raw/unnormalized
    FFT magnitude is O(sqrt(H*W)), i.e. in the tens/hundreds for typical patch sizes) and the same
    radial-distance mask (an unmasked magnitude L1 is dominated by a few huge low-frequency/DC bins).
    ...
    if p.dim() == 4:  # 3D batch: (B, D, H, W)
        _, D, H, W = p.shape
        fft_p = torch.fft.rfftn(p, dim=(-3, -2, -1), norm="ortho")
        fft_t = torch.fft.rfftn(t, dim=(-3, -2, -1), norm="ortho")
        freq_d = torch.fft.fftfreq(D, device=p.device)
        freq_h = torch.fft.fftfreq(H, device=p.device)
        freq_w = torch.fft.rfftfreq(W, device=p.device)
        FD, FH, FW = torch.meshgrid(freq_d, freq_h, freq_w, indexing="ij")
        radius = torch.sqrt(FD**2 + FH**2 + FW**2)
    else:  # 2D batch: (B, H, W)
        _, H, W = p.shape
...
class CycleGanLoss(nn.Module):
    ...
    def forward_discriminator(self, d_real, d_fake, real_images=None):
        ...
        if real_images is not None and self.r1_gamma > 0:
            total_loss = total_loss + self.calculate_r1_penalty(d_real, real_images)
