...
class Classification_Workflow(Base_Workflow):
    ...
    def __init__(self, cfg, job_identifier, device, args, **kwargs):
        super(Classification_Workflow, self).__init__(
    ...
    def define_metrics(self):
        ...
        """
        self.metrics = [
            Accuracy(task="multiclass", num_classes=self.cfg.MODEL.N_CLASSES)
        ]
        self.metric_names = ["accuracy"]
        if self.cfg.MODEL.N_CLASSES > 5:
            self.metrics.append(
                Accuracy(
                    task="multiclass", num_classes=self.cfg.MODEL.N_CLASSES, top_k=5
                )
            )
        ...
        if self.cfg.TRAIN.ENABLE:
            ...
            if self.cfg.DATA.TRAIN.IN_MEMORY:
                ...
                f_name = (
                    ...
                    else load_3d_data_classification
                )
                preprocess_cfg = (
            ...
            if not self.cfg.DATA.TEST.USE_VAL_AS_TEST:
                ...
                if self.test_filenames is None:
                    ...
                    for c_num, folder in enumerate(self.class_names):
                        self.test_filenames += sorted(
                            next(
                                os.walk(os.path.join(self.cfg.DATA.TEST.PATH, folder))
                            )[2]
                        )
            else:
                ...
                if self.cross_val_samples_ids is None:
                    ...
                    for c_num, folder in enumerate(self.class_names):
                        ids = sorted(
                            next(
                                os.walk(os.path.join(self.cfg.DATA.TRAIN.PATH, folder))
                            )[2]
                        )
    ...
    def print_stats(self, image_counter):
        ...
        if self.cfg.DATA.TEST.LOAD_GT and self.cfg.TEST.EVALUATE:
            ...
            if self.class_names is not None:
                ...
                display_labels = [
                    "Category {}".format(i) for i in range(self.cfg.MODEL.N_CLASSES)
                ]
            print(
                "\n"
                + classification_report(
                    self.all_gt, self.all_pred, target_names=display_labels
                )
            )
