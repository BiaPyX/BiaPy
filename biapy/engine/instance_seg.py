...
class Instance_Segmentation_Workflow(CellposeTestPhaseMixin, Base_Workflow):
    ...
    def __init__(self, cfg, job_identifier, device, system_dict, args, **kwargs):
        ...
        **kwargs : dict
        ...
        # GT_PATH is unused (co-located Zarr/H5 GT); walk PATH instead. Not for "synapses", which
        # still repoints GT_PATH at its own cached channel dir in prepare_instance_data().
    ...
    def define_activations_and_channels(self):
        ...
        for i, channel in enumerate(self.cfg.PROBLEM.INSTANCE_SEG.DATA_CHANNELS):
                if channel in ["B", "F", "P", "C", "T", "M", "F_pre", "F_post", "F_cleft"]:
                    ...
                    self.head_activations.append(dst.get("D", {}).get("act", "tanh"))
                    if set_model_output_channels:
                        self.model_output_channels[0] += 1
                        self.model_output_channel_info[0] += "+" + channel
                elif channel == "A":
