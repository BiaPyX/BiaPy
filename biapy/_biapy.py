...
class BiaPy:
    ...
    def export_model_to_bmz(
        ...
        # # Input normalization made with bioimageio package 
        # from bioimageio.core import create_prediction_pipeline
        # pipeline = create_prediction_pipeline(model_descr)
        # from bioimageio.core.digest_spec import get_test_inputs, get_test_outputs
        # bmz_input = get_test_inputs(model_descr)
        # pipeline.apply_preprocessing(bmz_input) # apply preprocessing 
        # bmz_manual_input = np.array(bmz_input.members["input0"].data)
        # print("BMZ input (manually norm): {}".format(bmz_manual_input[0,0,:5,0]))
        # # BMZ input (manually norm): [0.         0.07058824 0.         0.03529412 0.        ]
        
        # reproduced_bmz_pred = pipeline.predict_sample_without_blocking(get_test_inputs(model_descr), skip_postprocessing=True)
        # reproduced_bmz_pred = np.array(reproduced_bmz_pred.members["output0"].data)
        # print("BMZ output (manual): {}".format(reproduced_bmz_pred[0,0,:5,0]))
        # # BMZ output (manual): [-2.8167467 -3.839605  -4.7271824 -5.076702  -5.0331593], skip_postprocessing=True

        # reproduced_bmz_pred = pipeline.predict_sample_without_blocking(get_test_inputs(model_descr), skip_postprocessing=False)
        # reproduced_bmz_pred = np.array(reproduced_bmz_pred.members["output0"].data)
        # print("BMZ output (manual): {}".format(reproduced_bmz_pred[0,0,:5,0]))
        # # BMZ output (manual): [0.5141027  0.50526214 0.5021934  0.50155044 0.501619  ], skip_postprocessing=False
        # import pdb; pdb.set_trace()
        # from biapy.data.data_manipulation import save_tif
        # save_tif(reproduced_bmz_pred.transpose(0,2,3,1), "/data/dfranco/", ["bmz_output.tif"])

        # real_bmz_in = get_test_inputs(model_descr)
        # real_bmz_in = np.array(real_bmz_in.members["input0"].data)
        # print("BMZ input: {}".format(real_bmz_in[0,0,:5,0]))
        # # BMZ input: [ 0. 18.  0.  9.  0.]

        # real_bmz_out = get_test_outputs(model_descr)
        # real_bmz_out = np.array(real_bmz_out.members["output0"].data)
        # print("BMZ output: {}".format(real_bmz_out[0,0,:5,0]))
        # # BMZ output: [0.05667261 0.02110609 0.00879778 0.00621081 0.00649132]

        # biapy_input = self.workflow.bmz_config["test_input"]
        # print("BiaPy input (not norm): {}".format(biapy_input[0,0,:5,0]))
        # # BiaPy input (not norm): [ 0. 18.  0.  9.  0.]
        # biapy_input_norm = self.workflow.bmz_config["test_input_norm"]
        # print("BiaPy input (normalized): {}".format(biapy_input_norm[0,0,:5,0]))
        # # BiaPy input (normalized): [0.         0.07058824 0.         0.03529412 0.        ]

        # input_biapy = torch.from_numpy(self.workflow.bmz_config["test_input_norm"]).to(self.workflow.device)
        # reproduced_biapy_pred = self.workflow.model(input_biapy)
        # print("BiaPy output (manual, without activations): {}".format(reproduced_biapy_pred[0,0,:5,0]))
        # # BiaPy output (manual, without activations): tensor([-2.8121, -3.8369, -4.7244, -5.0752, -5.0308]

        # reproduced_biapy_pred = self.workflow.apply_model_activations(reproduced_biapy_pred, training=False)
        # print("BiaPy output (manual): {}".format(reproduced_biapy_pred[0,0,:5,0]))
        # # BiaPy output (manual): tensor([0.0567, 0.0211, 0.0088, 0.0062, 0.0065],

        # biapy_output = self.workflow.bmz_config["test_output"]
        # print("BiaPy output: {}".format(biapy_output[0,0,:5,0]))
        # # BiaPy output: [0.0567, 0.0211, 0.0088, 0.0062, 0.0065]

        # result = np.absolute(np.array(reproduced_bmz_pred) - np.array(biapy_output))
        # print(f"result.max(): {result.max()}")
        # # 0.0006444156
        # print(f"biapy_output[0,0,437,90]: {biapy_output[0,0,437,90]} , reproduced_bmz_pred[0,0,437,90]: {reproduced_bmz_pred[0,0,437,90]}")
        # # biapy_output[0,0,437,90]: 5.21540641784668e-08 , reproduced_bmz_pred[0,0,437,90]: 1.648813486099243e-05
        # print(f"biapy_output[0,0,856,398]: {biapy_output[0,0,856,398]} , reproduced_bmz_pred[0,0,856,398]: {reproduced_bmz_pred[0,0,856,398]}")
        # # biapy_output[0,0,856,398]: 1.4350336790084839 , reproduced_bmz_pred[0,0,856,398]: 1.4347714185714722
        # print(f"result[0,0,437,90]: {result[0,0,437,90]} , result[0,0,856,398]: {result[0,0,856,398]}")
        # # result[0,0,437,90]: 1.6435980796813965e-05 , result[0,0,856,398]: 0.00026226043701171875

        # import pdb; pdb.set_trace()
        # from biapy.data.data_manipulation import save_tif
        # save_tif(img.transpose(0,2,3,1), "/data/dfranco/", ["biapy_output.tif"])
