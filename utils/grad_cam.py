...
def grad_cam_sample(input_model, image, predicted_class, layer_name, out_dir, n_classes=2):
    """Generates an image with the activation maps in charge of the class decision on a specific layer.
       ...
       Gradient-based Localization <https://arxiv.org/abs/1610.02391>`_.
       ...
       ::
           ...
           # Extract the activation maps responsive of selecting the foreground class (1) in a binary segmentation task
           # on the layer 'conv2d_16'. The image should be any image one could predict() on. Notice that the number of
       ...
       ::
           ...
           # Notice that, if you select the activation maps of the last layer of the network, for example, 'conv2d_18'
           # in 2D U-Net implementation of this project, the output should be the same as the prediction on the
           # complete image
