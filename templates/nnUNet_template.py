...
from sota_implementations.nnUNet_2018.custom_da_gen_nnUNet import ImageDataGenerator
from sota_implementations.nnUNet_2018.nnUNet_2d import nnUNet_2D
from metrics import jaccard_index_numpy, voc_calculation
from tensorflow.keras.callbacks import EarlyStopping
from tensorflow.keras.models import load_model
from PIL import Image
from tqdm import tqdm
from sota_implementations.nnUNet_2018.smooth_tiled_predictions_nnUNet import \
...
# Number of feature maps to use in the first level of the nnUNet
feature_maps = 32
...
n_classes = 1
# Adjust the metric used accordingly to the number of clases. 
metric = "conv2d_41_jaccard_index"
...
#model = nnUNet_2D((None, None, img_channels), feature_maps=feature_maps,
model = nnUNet_2D([img_height, img_width, img_channels], feature_maps=feature_maps,
                  k_init=kernel_init, optimizer=optimizer,
...
score_per_crop = model.evaluate(                                                
    [X_test], [np.array([resize(img, (2,2), order=0, anti_aliasing=False) for img in Y_test]),\
               np.array([resize(img, (4,4), order=0, anti_aliasing=False) for img in Y_test]),\
               np.array([resize(img, (8,8), order=0, anti_aliasing=False) for img in Y_test]),\
               np.array([resize(img, (16,16), order=0, anti_aliasing=False) for img in Y_test]),\
               np.array([resize(img, (32,32), order=0, anti_aliasing=False) for img in Y_test]),\
               np.array([resize(img, (64,64), order=0, anti_aliasing=False) for img in Y_test]),\
               np.array([resize(img, (128,128), order=0, anti_aliasing=False) for img in Y_test]),\
               Y_test], batch_size=batch_size_value, verbose=1)

print("score_per_crop: {}".format(score_per_crop))
loss_per_crop = score_per_crop[9]
jac_per_crop = score_per_crop[-1]

print("Making the predictions on test data . . .")
preds_test = np.array(model.predict(X_test, batch_size=batch_size_value, verbose=1)[-1])
...
Y_test_50ov = np.array(model.predict(X_test, batch_size=batch_size_value, verbose=1)[-1])     
...
print("Making the predictions on test data . . .")
preds_test_full = np.array(model.predict(X_test, batch_size=batch_size_value, verbose=1)[-1])
...
del zfil_preds_test, Y_test_ensemble, preds_test_full
