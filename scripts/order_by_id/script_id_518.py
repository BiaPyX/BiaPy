...
sys.path.insert(0, os.path.join(script_dir, '..', 'cheng_2017'))
#sys.path.insert(0, os.path.join(script_dir, '..', 'cheng_2017', 'tf_2.0_code'))
...
# Loss decay
def lr_decay_callback(epoch, lr):
    if epoch == 180 or epoch == 270:
        return lr*0.1
    else:
        return lr
lr_callback = tf.keras.callbacks.LearningRateScheduler(lr_decay_callback, verbose=0)
...
if extra_train_data != 0:
    ...
    X_val = np.vstack([X_val]*4)
    Y_val = np.vstack([Y_val]*4)
##########################
#    DATA AUGMENTATION   #
##########################
...
model = asymmetric_U_Net([img_height, img_width, img_channels],
              numInitChannels=num_init_channels, spatial_dropout=spatial_dropout,
              fixed_dropout=fixed_dropout_value, loss_type=loss_type, 
...
if load_previous_weights == False:
    ...
    print("validation_steps: {} {} {}".format(len(X_val), batch_size_value, math.ceil(len(X_val)/batch_size_value)))
    print("steps_per_epoch: {} {} {}".format(len(X_train), batch_size_value, math.ceil(len(X_train)/batch_size_value)))
...
if random_crops_in_DA == False:
    ...
    # Save output images
    print("Saving predicted images . . .")
    save_img(Y=bin_preds_test, mask_dir=result_bin_dir, prefix="test_out_ov_bin")
    save_img(Y=preds_test, mask_dir=result_no_bin_dir, prefix="test_out_ov_no_bin")
